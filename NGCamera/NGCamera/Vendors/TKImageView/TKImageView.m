...
@property (assign, nonatomic) CGPoint frameC;
@end
...
{
    _rotation += rotation > 0 ? 90 : -90;
    _rotation = (_rotation >= 360 || _rotation <= -360  ? 0 : _rotation);
    ...
    if (_rotation % 180 == 0) {
        ...
        if (_imageAspectRatio > 1) {
            self.width = _frameH * (_frameW / _frameH);
            self.height = _frameH ;
        } else {
            self.width = _frameW;
            self.height = _frameW * (_frameW / _frameH);
        }
...
- (UIImage *)currentCroppedImage
{
    ...
    void (^block)(void) = ^{
        CGFloat scaleFactor = WIDTH(self->_imageView) / self->_toCropImage.size.width;
        int x = MINX(self->_cropAreaView) / scaleFactor + 0.5;
        int y = MINY(self->_cropAreaView) / scaleFactor + 0.5;
        int w = WIDTH(self->_cropAreaView) / scaleFactor + 0.5;
        int h = HEIGHT(self->_cropAreaView) / scaleFactor + 0.5;
        
        croppedImage = [self->_toCropImage subImageWithRect:CGRectMake(x, y, w, h)];
